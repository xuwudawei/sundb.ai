...
class TiDBVectorStore(BasePydanticVectorStore):
    _session: Session = PrivateAttr()
    _owns_session: bool = PrivateAttr()
    _filter_document_ids: Optional[List[int]] = PrivateAttr(default=None)  # 新增文档过滤字段
    ...
    def __init__(
        ...
        filter_document_ids: Optional[List[int]] = None,  # 新增过滤参数
        **kwargs: Any
    ) -> None:
        super().__init__(**kwargs)
        self._session = session
        self._owns_session = session is None
        self._filter_document_ids = filter_document_ids  # 初始化过滤条件
        if self._session is None:
            self._session = Session(engine)
    ...
    def query(self, query: VectorStoreQuery, **kwargs: Any) -> VectorStoreQueryResult:
        ...
        # 新增文档ID过滤逻辑
        if self._filter_document_ids:
            stmt = stmt.where(DBChunk.document_id.in_(self._filter_document_ids))
