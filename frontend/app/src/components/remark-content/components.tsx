...
export const getRehypeReactOptions = ({
  portalContainer,
}: {
  ...
  components: {
    img({ src, alt, ...props }) {
      // If alt text is literally "Image", just render the src link instead of an <img>.
      if (alt === 'Image') {
              ...
              if (id) {
                ...
                if (li) {
                  const a = li.querySelector(
                    `a:first-child:not([data-footnote-backref])`
        ...
        >
          ...
          <HoverCardPortal container={portalContainer}>
            <HoverCardContent
              ...
              className="p-1 w-[200px] overflow-hidden rounded-lg border text-xs"
            >
    ...
    pre({ children, node, ...props }) {
      ...
      return (
        <pre {...props}>
          ...
          {isCodeBlock && (
            <div
              className="absolute right-1 top-1 transition-opacity opacity-30 hover:opacity-100"
              data-role="codeblock-addon"
            >
